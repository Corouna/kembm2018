/* THIS IS TO STORE ALL GLOBAL VARIABLES!!! */

var _USER = {
	firstName: 'John',
	lastName: 'Doe'
};

var _PRODUCTS = [
	{
		id: '1',
		title: 'Apple iPhone 8',
		description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Nisi tempora similique reiciendis, error nesciunt vero, blanditiis pariatur dolor, minima sed sapiente rerum, dolorem corrupti hic modi praesentium unde saepe perspiciatis.'
	},
	{
		id: '2',
		title: 'Apple iPhone 8 Plus',
		description: 'Velit odit autem modi saepe ratione totam minus, aperiam, labore quia provident temporibus quasi est ut aliquid blanditiis beatae suscipit odio vel! Nostrum porro sunt sint eveniet maiores, dolorem itaque!'
	},
	{
		id: '3',
		title: 'Apple iPhone X',
		description: 'Expedita sequi perferendis quod illum pariatur aliquam, alias laboriosam! Vero blanditiis placeat, mollitia necessitatibus reprehenderit. Labore dolores amet quos, accusamus earum asperiores officiis assumenda optio architecto quia neque, quae eum.'
	},
];

var _ATURCARA = [
	{
		tarikh: "21 September 2018",
		hari: "Jumaat",
		acara: [
			{
				jam: "3.00",
				waktu: "pm",
				aktiviti: "Urusan pendaftaran"
			},
			{
				jam: "6.30 - 7.30",
				waktu: "pm",
				aktiviti: "Makan malam"
			},
			{
				jam: "8.00 - 9.30",
				waktu: "pm",
				aktiviti: "Kebaktian malam Sabat"
			}
		]
	},
	{
		tarikh: "22 September 2018",
		hari: "Sabtu",
		acara: [
			{
				jam: "7.45 - 8.15",
				waktu: "am",
				aktiviti: "Sarapan"
			},
			{
				jam: "9.00 - 10.00",
				waktu: "am",
				aktiviti: "Perasmian Kem BM 2018"
			},
			{
				jam: "10.00 - 10.45",
				waktu: "am",
				aktiviti: "Pelajaran Sekolah Sabat"
			},
			{
				jam: "11.00 - 12.00",
				waktu: "pm",
				aktiviti: "Sesi pertama : Perkasakan Kasihmu"
			},
			{
				jam: "12.30 - 1.30",
				waktu: "pm",
				aktiviti: "Makan tengahari"
			},
			{
				jam: "2.45 - 4.00",
				waktu: "pm",
				aktiviti: "Sesi kedua : Perkasakan Kasihmu"
			},
			{
				jam: "4.00 - 6.00",
				waktu: "pm",
				aktiviti: "Acara belia"
			},
			{
				jam: "6.30 - 7.30",
				waktu: "pm",
				aktiviti: "Makan malam"
			},
			{
				jam: "8.00 - 9.30",
				waktu: "pm",
				aktiviti: "Sesi ketiga : Perkasakan Kasihmu"
			},
			{
				jam: "9.30 - 11.00",
				waktu: "pm",
				aktiviti: "Free time"
			}
		]
	},
	{
		tarikh: "23 September 2018",
		hari: "Ahad",
		acara: [
			{
				jam: "7.45 - 8.15",
				waktu: "am",
				aktiviti: "Sarapan"
			},
			{
				jam: "8.30 - 10.30",
				waktu: "am",
				aktiviti: "Aktiviti outdoor"
			},
			{
				jam: "11.00 - 12.00",
				waktu: "pm",
				aktiviti: "Acara penutupan Kem BM 2018"
			},
			{
				jam: "12.30 - 1.30",
				waktu: "pm",
				aktiviti: "Makan tengahari"
			},
			{
				jam: "1.30",
				waktu: "pm",
				aktiviti: "Selesai!!!"
			}
		]
	}
];

var _PUJIAN = [
	{
		tajuk: "Lagu tema - Bapa Yang Kekal",
		id: 1,
		lirik: [
			[
				'Kasih yang sempurna telah ku terima dari-Mu',
			 	'Bukan kerna kebaikanku hanya', 
				'Oleh kasih karunia-Mu',
				'Kau pulihkan aku',
 				'layakkanku tuk dapat memanggil-Mu, Bapa'
			],
			[
				'Kau beri yang kupinta',
				'Saatku mencari ku mendapatkan',
				'Ku ketuk pintu-Mu dan Kau bukakan',
				'Sebab Kau Bapaku, Bapa yang kekal',
				'Takkan Kau biarkan aku melangkah hanya sendirian',
				'Kau selalu ada bagiku', 
				'Sebab Kau bapaku Bapa yang kekal'
			]
		]
	},
	{
		tajuk: "Kasih Yesus",
		id: 2,
		lirik: [
			[
				'Kasih Yesus indah indah oh indah',
				'Kasih Yesus indah indah oh indah',
				'Lebih indah dari pelangi',
				'Lebih indah dari bintang dilangit',
				'Lebih indah dari bunga ditaman, oh Yesusku',
				'Lebih indah dari pelangi',
				'Lebih indah dari bintang dilangit',
				'Lebih indah dari bunga ditaman, oh Yesusku'
			]
		]

	},
	{
		tajuk: "Ku Daki Daki",
		id: 3,
		lirik: [
			[
				'Ku daki daki daki daki gunung yang tinggi',
				'Ku selam selam selam selam laut samudra',
				'Ku merentasi padang rumput luas terbentang',
				'Yesus bersertaku'
			],
			[
				'Ku turun turun turun turun cerun yang curam',
				'Ku terbang terbang terbang terbang atas angkasa',
				'Ku dayung dayung dayung dayung sampai ke seberang',
				'Yesus bersertaku'
			],
			[
				'(cho)',
				'Di kanan Kau ada (ohh), di kiri Kau ada (ohh)',
				'Di atas dan di bawah kau ada, oh oh ohhhh',
				'Disuka Kau ada, diduka pun Kau ada',
				'Kerna engkaulah Yesus Tuhan'
			]
		]
	},
	{
		tajuk: "Bersorak-sorailah",
		id: 4,
		lirik: [
			[
				'Bersorak sorailah, bagi Allah',
				'Kekuatan kita bersorak sorailah, bagi Allah kita'
			],
			[
				'Angkat lagu, bunyikanlah rebana',
				'Kecapi yang merdu, diiringi gambus',
				'Tiuplah sangkakala pada bulan baru',
				'Pada bulan purnama ohhhh',
				'Pada hari ini'
			]
		]
	},
	{
		tajuk: "Kepala Leher",
		id: 5,
		lirik: [
			[
				'(cho)',
				'Kepala leher, bahu dan pinggang',
				'Lutut dan kaki, lutut dan kaki',
				'Kaki dan lutut, pinggang dan bahu',
				'Leher kepala, Yesus yang punya'
			],
			[
				'Anak monyet diatas pokok', 
				'Anak cacing didalam tanah',
				'Anak burung didalam sarang', 
				'Anak Tuhan didalam gereja'
			],
			[
				'Panjang muka namanya kuda', 
				'Panjang hidung namanya gajah',
				'Panjang tangan itu pencuri',
				'Panjang sabar itu anak Tuhan'
			]
		]
	},
	{
		tajuk: "Kita Satu Dalam Tuhan",
		id: 6,
		lirik: [
			[
				'Kita satu dalam Tuhan',
				'Kita satu dalam Tuhan',
				'Kita disatukan di dalam Roh Tuhan',
				'Kita satu dalam Tuhan'
			],
			[
				'Mari kita bernyanyi',
				'Rasakan kasih Tuhan',
				'Mari berpegang tangan, supaya dunia tahu',
				'Kita satu dalam Tuhan'
			]
		]
	},
	{
		tajuk: "Oh Tuhan Pimpinlah Langkahku",
		id: 7,
		lirik: [
			[
				'Oh Tuhan pimpinlah langkahku',
				'Ku tak berani jalan sendiri',
				'SertaMu itulah doaku',
				'Ajarku merendahkan diri'
			],
			[
				'Menurut FirmanMu setiap hari',
				'Jadikan pelita dalam gelap',
				'Mencari domba yang sesat',
				'Itulah kerinduan jiwaku'
			]
		]
	},
	{
		tajuk: "Satu Anak Tuhan",
		id: 8,
		lirik: [
			[
				'Satu anak Tuhan pergi sekolah Sabat',
				'Satu anak Tuhan bawa teman pergi sekolah Sabat'
			],
			[
				'Dua anak Tuhan pergi sekolah Sabat',
				'Dua anak Tuhan bawa teman pergi sekolah Sabat'
			],
			[
				'Tiga anak Tuhan pergi sekolah Sabat',
				'Tiga anak Tuhan bawa teman pergi sekolah Sabat'
			],
			[
				'Empat anak Tuhan pergi sekolah Sabat',
				'Empat anak Tuhan bawa teman pergi sekolah Sabat'
			],
			[
				'Lima anak Tuhan pergi sekolah Sabat',
				'Lima anak Tuhan bawa teman pergi sekolah Sabat'
			]
		]
	},
	{
		tajuk: "Hari Ini",
		id: 9,
		lirik: [
			[
				'Hari ini, ku rasa bahagia',
				'Berkumpul bersama saudara seiman',
				'Tuhan Yesus menyatukan kita',
				'Tanpa memandang diantara kita'
			],
			[
				'Bergandingan tangan, dalam kasih, dalam satu hati',
				'Berjalan dalam terang kasih Tuhan'
			],
			[
				'Kau sahabatku dan kau saudaraku',
				'Tiada yang dapat memisahkan kita semua ohh',
				'Kau sahabatku dan kau saudaraku',
				'Tiada yang dapat memisahkan kita semua',
				'Tiada yang dapat memisahkan kita'
			]
		]
	},
	{
		tajuk: "Hati Yang Gembira",
		id: 10,
		lirik: [
			[
				'Hati yang gembira adalah obat',
				'Seperti obat hati yang sedih',
				'Tapi semangat yang patah keringkan tulang',
				'Hati yang gembira Tuhan senang'
			]
		]
	},
	{
		tajuk: "Bila Ku Senang",
		id: 11,
		lirik: [
			[
				'Bila ku senang, hatiku bahagia',
				'Bagaimana cara menyatakannya',
				'Tuhan berikan tubuh kepadaku',
				'Nyatakan semua dengan tubuhku',
				'Dengan tangan, dengan kaki',
				'Dengan badan dan gerakkan.. Ah ahh'
			],
			[
				'(Pertama dengan tangan.. putarkan ke depan, putarkan ke bawah, ke kanan ke kiri)',
				'(Kedua dengan kaki.. langkahkan ke depan, langkahkan ke belakang, ke kanan ke kiri)',
				'(Ketiga dengan badan.. pegang bahu kawan, tolakkan kebawah, ke kanan ke kiri)'
			]
		]
	},
	{
		tajuk: "Pasir",
		id: 12,
		lirik: [
			[
				'Mari semua pegang tangan, angkat tinggi-tinggi',
				'Jadi satu hati',
				'Lupakanlah rasa benci, atau iri hati',
				'Jadi satu hati'
			],
			[
				'(cho)',
				'Jangan jadi seperti pasir',
				'Kerna pasir mudah ditiup angin',
				'Jangan jadi seperti pasir',
				'Kerna pasir mudah ditiup angin'
			]
		]
	},
	{
		tajuk: "Mukjizat Itu Nyata",
		id: 13,
		lirik: [
			[
				[
					'Tak terbatas kuasaMu Tuhan',
					'Semua dapat Kau lakukan',
					'Apa yang kelihatan mustahil bagiku',
					'Itu sangat mungkin bagiMu'
				],
				[
					'(cho)',
					'Di saat ku tak berdaya',
					'KuasaMu yang sempurna',
					'Ketika ku percaya',
					'Mukjizat itu nyata',
					'Bukan karna kekuatan',
					'Namun RohMu, ya Tuhan',
					'Ketika ku berdoa',
					'Mukjizat itu nyata.'
				]
			]
		]
	},
	{
		tajuk: "Tuhan Pasti Sanggup",
		id: 14,
		lirik: [
			[
				'Kuatkanlah hatimu',
				'Lewati setiap persoalan',
				'Tuahn Yesus selalu menompangmu',
				'Jangan berhenti harap padaNya'
			],
			[
				'Tuhan pasti sanggup',
				'Tangan-Nya takkan terlambat tuk mengangkatmu',
				'Tuhan pasti sanggup',
				'Percayalah dia tak tinggalkanmu'
			],
			[
				'Percayalah dia akan mengangkatmu'
			]
		]
	},
	{
		tajuk: "Bapa Sentuh Hatiku",
		id: 15,
		lirik: [
			[
				'Betapaku mencintai, segala yang telah terjadi',
				'Tak pernah sendiri jalani hidup ini, selalu menyertai',
				'Betapa ku menyadari di dalam hidupku ini',
				'Kau selalu memberi rancangan terbaik',
				'Oleh karena kasih'
			],
			[
				'(cho)',
				'Bapa, sentuh hatiku, ubah hidupku',
				'Menjadi yang baharu',
				'Bagai, emas yang murni',
				'Kau membentuk bejana hatiku'
			],
			[
				'Bapa, ajarku mengerti sebuah kasih',
				'Yang selalu memberi',
				'Bagai, air mengalir',
				'Yang tiada pernah berhenti'
			]
		]
	},
	{
		tajuk: "Dia Hanya Sejauh Doa",
		id: 16,
		lirik: [
			[
				'Bila kau rasa gelisa dihatimu',
				'Bila kelam kabut tak menentu hidupmu',
				'Ingat masih ada seorang penolong bagimu',
				'Yesus tak pernah jauh darimu'
			],
			[
				'Bila cobaan mengodai hatimu',
				'Bila sengsara menimpa keadaanmu',
				'Ingat Yesus takkan pernah jauh darimu',
				'Dia selalu pedulikan kamu'
			],
			[
				'(cho)',
				'Berseru memanggil namaNya',
				'Berdoa Diakan segera menghampiri dirimu',
				'Percaya Dia tak jauh darimu',
				'Dia hanya sejauh doa'
			]
		]
	},
	{
		tajuk: "Lingkupiku",
		id: 17,
		lirik: [
			[
				'Lingkupiku, dengan sayap-Mu',
				'naungiku, dalam kuasa-Mu',
			],
			[
				'(cho)',
				'Disaat badai bergelora',
				'ku akan terbang bersama-Mu',
				'Bapa kau raja atas semesta',
				'ku tenang sebab Kau Tuhanku'
			]
		]
	},
	{
		tajuk: "Bagaikan Bejana",
		id: 18,
		lirik: [
			[
				'Bagaikan bejana siap dibentuk', 
				'Demikian hidupku ditanganMu',
				'Dengan urapan kuasa rohMu',
				'Ku dibaharui selalu'
			],
			[
				'Jadikan ku alat dalam rumahMu',
				'Inilah hidupku di tanganMu',
				'Bentuklah s`turut kehendakMu', 
				'Pakailah sesuai rencanaMu'
			],
			[
				'(cho)',
				'Ku mau sepertiMu Yesus', 
				'disempurnakan selalu',
				'Dalam segenap jalanku', 
				'Memuliakan namaMu.'
			]
		]
	},
	{
		tajuk: "Jangan Lelah",
		id: 19,
		lirik: [
			[
				'Jangan lelah, bekerja di ladangNya Tuhan',
				'Roh Kudus yang b’ri kekuatan, yang mengajar dan menopang'
			],
			[
				'Tiada lelah, bekerja bersamaMu Tuhan',
				'Yang selalu mencukupkan akan segalanya'
			],
			[
				'(cho)',
				'Ratakan tanah bergelombang',
				'Timbunlah tanah yang berlubang',
				'Menjadi siap dibangun',
				'Di atas dasar iman'
			]
		]
	},
	{
		tajuk: "Penjala Manusia",
		id: 20,
		lirik: [
			[
				'Saat itu ku dengar bisikian Roh Kudus',
				'Ku keliru akan hala tuju ku',
				'Adakah ku pilih keinginan dunia',
				'Adakah ku pilih kemasyhuran'
			],
			[
				'Tuhan memanggil diri ku',
				'Sperti Dia memanggil rasulNya dulu',
				'Tuk belajar melakukan dan mengajar firmanNya',
				'Lalu ku dikuatkan oleh RohNya'
			],
			[
				'Pergilah jadikanlah semua bangsa muridKu',
				'Dan baptislah mereka',
				'Dalam nama Bapa, Anak dan Roh Kudus',
				'Pergilah jadikanlah semua bangsa muridKu',
				'Dan baptislah mereka',
				'Dalam nama Bapa, Anak dan Roh Kudus'
			],
			[
				'Rendahkanlah hatiku ya Tuhan',
				'Pabila Roh Mu berbisik',
				'Briku keinginan tuk menyambut',
				'panggilanMu',
				'Sebagai Penjala Manusia'
			],
			[
				'Saat ini ku dengar lagi bisikan Mu',
				'Tuhan ! Sudahkah ku guna talentaku',
				'Sudahkah Ku melakukan kehendak Mu Tuhan',
				'Sebagai Penjala Manusia'
			]
		]
	},
	{
		tajuk: "Kasih Bapa",
		id: 21,
		lirik: [
			[
				'Seperti mentari yang bersinar, seperti itu kasih Bapa',
				'Seperti gelombang samudra , takkan pernah berhenti Kau mengasihiku',
				'Seperti tingginya langit biru, demikian tinggi kasih Bapa',
				'Sedalam lembah bayang maut, demikian dalam Kau tebus hidupku'
			],
			[
				'(cho)',
				'KasihMu lebih dari mentari',
				'Yang tak pernah berhenti memancarkan sinarnya',
				'CintaMu lebih dari samudera',
				'Tenggelam ku didalam kesetiaanMu Tuhan',
				'Terima kasih atas cintaMu'
			]
		]
	},
];